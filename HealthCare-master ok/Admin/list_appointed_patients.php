...
<style>
  ...
  body {
    ...
    width: 100%;
    height: 100vh;
    background-image: url(../pic/Ap.jpeg);
  ...
  <form action="delete_user_appointment_request.php" method="POST">
    <table border="1" cellPadding="13" align="center" class="table table-hover table-dark">
      <thead>
        <tr>
          ...
          <th class="text-center text-dark active font-weight-bold">
            <h3>Doctor's Name</h3>
          </th>
          <th class="text-center text-dark active font-weight-bold">
	...
	from doctor join schedule on schedule.d_id=doctor.id join appointment on appointment.sid=schedule.s_id  join user on appointment.pid=user.id where  appointment.permission='Approved'";
      ...
      ?>
        <tr>
          <th>
            <h3><?php echo "<a href='detail.php?s_id={$drow['s_id']}'>{$drow['dfname']}{$drow['dlname']}</a>";  ?>&nbsp; &nbsp;Specialist Of &nbsp;&nbsp; <?php echo $drow['specialist'];    ?></h3>
          </th>

          <th>
            <h3><?php echo $drow['booking_date'];    ?></h3>
          </th>
          <th>
            <h3><?php echo $drow['booking_time'];    ?></h3>
          </th>
          <th>
            <h3><?php echo  "<a href='user_detail.php?id={$drow['id']}'>{$drow['ufname']}{$drow['ulname']}</a>"; ?></h3>
          </th>
